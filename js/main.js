...
function init() {
particles = [];
var count = Math.max(18, Math.min(55, Math.round(W / 25)));
...
var animId, frameSkip = false;
function step() {
if (!ready) { animId = requestAnimationFrame(step); return; }
frameSkip = !frameSkip;
...
if (!ready && W > 0 && H > 0) { ready = true; step(); }
}
var rt;
...
var w = entries[0].contentBoxSize ? entries[0].contentBoxSize[0].inlineSize : entries[0].contentRect.width;
var h = entries[0].contentBoxSize ? entries[0].contentBoxSize[0].blockSize : entries[0].contentRect.height;
setSize(w, h);
if (!ready && w > 0 && h > 0) { ready = true; init(); step(); }
}).observe(hero);
}
requestAnimationFrame(function () {
if (!ready) build();
